...
function mapping(data){disks=data;if(!$('bay-mapping')||!settings)return;const known=[...data.bays.map(b=>b.disk).filter(Boolean),...data.unassigned];$('bay-mapping').innerHTML=data.bays.map(b=>`<tr><td>BAY ${String(b.bay).padStart(2,'0')}</td><td><select data-bay="${b.bay}" aria-label="BAY ${b.bay} 磁盘"><option value="">空 / 解除绑定</option>${known.map(d=>`<option value="${escape(d.serial)}" ${d.serial===b.serial?'selected':''}>${escape(d.model)} · ${escape(d.serial)}${d.online?'':'（离线）'}</option>`).join('')}${b.serial&&!known.some(d=>d.serial===b.serial)?`<option selected value="${escape(b.serial)}">${escape(b.serial)}（尚未发现）</option>`:''}</select></td><td><div class="toolbar"><button data-save-bay="${b.bay}" class="small">保存</button><button data-delete-bay="${b.bay}" class="small danger" ${b.serial?'disabled title="请先解除磁盘绑定"':''}>删除</button></div></td></tr>`).join('');}
function render(){
 ...
 for(const [path,label,type] of [['cpuTemperatureSensorId','CPU 温度传感器','Temperature'],['fanRpmSensorId','风扇 RPM 传感器','Fan'],['fanControlSensorId','可写风扇控制器','Control']]){
  const matching=sensors.filter(s=>s.type===type&&(type!=='Control'||s.writable)).sort((a,b)=>(b.value>0)-(a.value>0));const unit=type==='Temperature'?' °C':type==='Fan'?' RPM':' %';addField('sensor-fields',`<label for="${path}">${label}</label><select id="${path}"><option value="">${type==='Control'?'未绑定（禁止写入）':'自动发现（只读）'}</option>${matching.map(s=>`<option value="${escape(s.id)}" ${s.id===settings[path]?'selected':''}>${escape(s.name)} · ${s.value==null?'无读数':Number(s.value).toFixed(1)+unit} · ${escape(s.id)}</option>`).join('')}${settings[path]&&!matching.some(s=>s.id===settings[path])?`<option selected value="${escape(settings[path])}">${escape(settings[path])}（未发现）</option>`:''}</select>`);
 }
...
$('add-bay').onclick=()=>run($('add-bay'),async()=>{const result=await api('/api/settings/bays','POST');mapping(await api('/api/disks'));toast('已增加 BAY '+result.bay);});
$('bay-mapping').onclick=e=>{const save=e.target.closest('[data-save-bay]');if(save)return run(save,async()=>{const bay=save.dataset.saveBay;const serial=document.querySelector(`[data-bay="${bay}"]`).value;await api(`/api/settings/bays/${bay}`,'PUT',{serial:serial||null});toast('BAY '+bay+' 映射已保存');mapping(await api('/api/disks'));});const remove=e.target.closest('[data-delete-bay]');if(remove)run(remove,async()=>{const bay=remove.dataset.deleteBay;await api(`/api/settings/bays/${bay}`,'DELETE');mapping(await api('/api/disks'));toast('已删除 BAY '+bay);});};
